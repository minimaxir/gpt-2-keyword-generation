...
def build_pattern(sections, start_token, end_token):
    ...
    for section in sections:
        pattern_text += (
            "(?:{})".format(re.escape(DELIMS["section"] + DELIMS[section])) + "(.*)"
        )
...
def decode_texts(
    texts, sections=["title"], start_token="<|startoftext|>", end_token="<|endoftext|>"
):
    ...
    for text in texts:
        ...
        if decoded_text is None:
            continue
        decoded_text_attrs = tuple(decoded_text.group(i) for i in group_indices)
...
def decode_file(
    ...
    assert (
        len(sections) == 1
    ), "This function only supports output of a single section for now."
    doc_pattern = re.compile(
        re.escape(start_token) + "(.*)" + re.escape(end_token), flags=re.MULTILINE
    )
    ...
    decoded_docs = decode_texts(
        docs, sections=sections, start_token=start_token, end_token=end_token
    )
